...
local function disable_non_desert_tiles()
...
--- Floors remove decal
local function tiles_remove_decal()
	for _, tile_name in pairs(data.raw["tile"]) do
...
-- Remove Water gen
local function remove_water_autoplace()
...
local function disable_certen_trees()
...
local function add_Collision_Layer(entity, layer)
	...
	if entity.collision_mask then
		...
		entity.collision_mask = {"water-tile", "player-layer", layer} 
	end
end


local worm_collision_layer = collision_mask_util.get_first_unused_layer() --"layer-14"
...
local function floor_worm_collision_mask()
	--data.raw["simple-entity"]["wall-interface"].collision_mask = { worm_collision_layer }
...
-- Add collision check to tiles
local function tiles_add_collision_check()
	for _, tile_name in pairs(data.raw["tile"]) do
		if vanilla_floor_tile_names[tile_name.name] then
			tile_name.check_collision_with_entities = true
		end
	end  
end 
